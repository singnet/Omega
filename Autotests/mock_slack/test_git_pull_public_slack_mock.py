...
access to GitHub вЂ” only the LLM dispatch is mocked.
...
def test_git_pull_public_slack_mock(llm, sl):
    remote = get_git_remote()

    with Checker("git pull public (slack mock)", cleanup_dirs=[TARGET_DIR]) as c:
        ...
        # ./tmp_clone shim then move contents вЂ” simpler: rm -rf then
